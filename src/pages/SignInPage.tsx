...
import { signInWithEmail, sendPhoneOtp, verifyPhoneOtp } from '@/lib/auth';
  ...
  return (
    <div className="flex min-h-[100dvh] items-center justify-center bg-[#0D0A07] px-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md bg-[#1A1410] border border-[#C9A84C]/25 rounded-md p-8 md:p-10">
        <h1 className="text-2xl font-display text-[#C9A84C] text-center mb-2">Welcome Back</h1>
        <p className="text-[#F5F0E8]/60 text-center text-sm mb-8">Sign in to access your account</p>
        ...
        <p className="text-center text-sm text-[#F5F0E8]/60 mt-8">
          ...
          <Link href={`/sign-up?redirect=${encodeURIComponent(redirectTo)}`} className="text-[#C9A84C] hover:underline">
            Create one
          </Link>
        </p>
      </div>
