...
-- Drives the sequential Staff ID format: a fixed "30214" prefix followed by
-- a 5-digit zero-padded counter (3021400000, 3021400001, ...), assigned the
-- moment someone redeems a staff invite code below.
...
begin
  if auth.uid() is null then
    return json_build_object('ok', false, 'error', 'You need to be signed in to redeem an invite code. Please confirm your email and sign in, then try again.');
  end if;
  ...
  next_seq := nextval('public.staff_id_seq');
  new_staff_id := '30214' || lpad(next_seq::text, 5, '0');
...
-- Signed-in customers can submit their own quote request and see their own
-- past requests. Anonymous visitors can't submit one here (unlike the
-- general Contact form) because a quote request needs a way to be matched
-- back to a customer account for follow-up.
...
-- ============================================================
-- 5b. IN-APP NOTIFICATIONS (the Account page bell — separate from the
-- WhatsApp/SMS alerts above, which go to the business owner, not customers)
-- ============================================================
...
-- Only server-side triggers insert notifications (via security definer
-- functions below) -- no client-side insert policy exists on purpose, so a
-- customer can never write a fake notification to their own feed.
...
-- Notify the customer when their booking's status actually changes (not on
-- the initial insert -- they already know they just submitted it).
...
-- Notify the customer when staff/admin replies to or resolves their quote request.
...
-- The public only ever sees approved testimonials.
create policy "Anyone can view approved testimonials"
  on public.testimonials for select
  using (approved = true);

-- A signed-in customer can submit their own testimonial (always starts
-- unapproved — see trigger below — so nothing customer-written appears on
-- the site until staff/admin reviews it).
create policy "Signed-in users can submit their own testimonial"
  on public.testimonials for insert
  with check (auth.uid() = author_id);

-- Staff/admin can see every testimonial (including unapproved ones, so they
-- have something to moderate) and manage them (approve, edit, delete).
create policy "Staff and admin manage all testimonials"
  on public.testimonials for all
  using (public.is_staff_or_admin(auth.uid()))
  with check (public.is_staff_or_admin(auth.uid()));

-- Force every customer-submitted testimonial to start unapproved, regardless
-- of what the client sends — prevents a customer from just setting
-- approved=true themselves in the insert payload.
